...
public abstract class ExportTrackTask  extends AsyncTask<Void, Long, Boolean> {
	...
	/**
	 * Writes the GPX file
	 * @param tags The tags assigned to the track
	...
	private void writeGpxFile(String tags, String track_description, Cursor cTrackPoints, Cursor cWayPoints, File target) throws IOException {
		...
		String gpxFormat = PreferenceManager.getDefaultSharedPreferences(context).getString(
				OSMTracker.Preferences.KEY_GPX_FORMAT,
				OSMTracker.Preferences.VAL_GPX_FORMAT
		);
		...
		try {
			...
			switch (gpxFormat) {
				case "long_format": {
					...
					writeTrackPoints_long(context.getResources().getString(R.string.gpx_track_name), tags, track_description, writer, cTrackPoints, fillHDOP, compassOutput);
					break;
				}
				case "short_format": {
	...
	/**strict
	 * Iterates on track points and write them - long version.
	 ...
	 * @throws IOException
	 */
	private void writeTrackPoints_long(String trackName, String tags, String track_description, Writer fw, Cursor c, boolean fillHDOP, String compass) throws IOException {
	...
	/**
	 * Iterates on way points and write them - long version.
	 * @param fw Writer to the target file.
